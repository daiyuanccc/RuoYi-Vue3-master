<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['elder:plan:add']">新增</el-button>
      </el-col>
    </el-row>
      ...
      <el-table-column label="操作" align="center" fixed="right" width="280" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" :class="scope.row.count ? 'disabled' : ''"
            @click="handleUpdate(scope.row)" v-hasPermi="['elder:plan:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" :class="scope.row.count ? 'disabled' : ''"
            @click="handleDelete(scope.row)" v-hasPermi="['elder:plan:remove']">删除</el-button>
          <el-button link type="primary" icon="Delete" @click="handleLook(scope.row)"
            v-hasPermi="['elder:plan:remove']">查看</el-button>
    ...
    <pagination v-show="total > 0" :total="total" v-model:page="queryParams.pageNum"
      v-model:limit="queryParams.pageSize" @pagination="getNursingPalnList" />
            ...
            <div class="tableBody">
              <div class="tableColumn" v-for="(item, index) in nursingPalnList" :key="index">
                ...
                <div class="column" v-if="!isLook">
                  <el-icon v-if="nursingPalnList.length > 1" @click="handleRowDel(item, index)" class="delect">
                    <Minus />
                  </el-icon>
...
/** 查询护理计划列表 */
const getNursingPalnList = async () => {
  loading.value = true;
  try {
    const response = await listNursingPlan(queryParams.value);
    planList.value = response.rows;
    total.value = response.total;
  } catch (error) {
    proxy.$modal.msgError('获取护理计划列表失败');
...
const getDetails = (id) => {
  getNursingPlan(id).then((response) => {
    ...
    // 添加调试确认ID是否正确加载
    console.log('加载详情ID:', formData.value.id);
  });
}
/** 提交按钮 */
function submitForm() {
