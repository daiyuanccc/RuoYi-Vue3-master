...
const getAllFloorList = async () => {
  ...
  if (res.code === 200) {
    ...
    // 过滤掉undefined或null的数据
    tabData.value = res.data.filter(item => item && typeof item === 'object');
    ...
    if (tabData.value.length > 0) {
      tabData.value.forEach((val, index) => {
        // 添加val的验证
        if (!val) {
          console.warn('发现undefined的楼层项，跳过处理');
          return;
        }
        
        // 新增编辑楼层逻辑
        if (floorBase && floorBase.code) {
          if (floorBase.code > val.code) {
            tabBar.value.currentId = index + 1;
            floorId.value = tabData.value[index + 1]?.id; // 使用可选链
          } else if (
          ...
          if (index === floorIndex.value) {
            floorId.value = tabData.value[floorIndex.value]?.id; // 使用可选链
          }
          if (isOperateSuccess.value && floorId.value !== val.id) {
            ...
            if (tabData.value.length === floorIndex.value) {
              ...
              floorId.value = tabData.value[floorIndex.value - 1]?.id; // 使用可选链
            }
            // 删除的是第一条逻辑
            if (index === floorIndex.value) {
              floorId.value = tabData.value[floorIndex.value]?.id; // 使用可选链
...
const handleFloorDeleteSubmit = async () => {
  ...
  if (res.code === 200) {
    ...
    proxy.$modal.msgSuccess('删除成功'); // 删除成功提示
  }
};
// 房间******************************************
// 根据不同的楼层id获取房间数据
const getRoomListData = async () => {
  // 添加参数验证
  console.log('当前floorId值:', floorId.value);
  console.log('floorId类型:', typeof floorId.value);
  
  if (!floorId.value || floorId.value === 'getRoomsByFloorId') {
    console.error('floorId值不正确:', floorId.value);
    // 尝试从tabData中获取第一个有效的楼层ID
    if (tabData.value && tabData.value.length > 0 && tabData.value[0]) {
      floorId.value = tabData.value[0].id;
      console.log('已重置floorId为:', floorId.value);
    } else {
      console.error('无法获取有效的楼层ID');
      return;
    }
  }
  
  // 确保floorId是数字类型
  const floorIdNum = Number(floorId.value);
  if (isNaN(floorIdNum)) {
    console.error('floorId无法转换为数字:', floorId.value);
    return;
  }
  
  try {
    const res = await getRoomBedList(floorIdNum);
    if (res.code === 200) {
      console.log('获取到的房间数据:', res.data);
...
const handleDeleteSubmit = async () => {
  ...
  if (type === 2) {
    await handleRoomDeleteSubmit()
  } else if (type === 3) {
    await handleBedDeleteSubmit()
  } else {
    await handleFloorDeleteSubmit()
  }
}
</script>
